...
RUN apt-get install -y wget tar mariadb-server
RUN apt-get install -y nginx php7.3 php7.3-mysql php7.3-fpm php7.3-bcmath php7.3-curl php7.3-gd php7.3-imagick php7.3-mbstring php7.3-soap php7.3-xml php7.3-zip
...
RUN /init_db.sh && mariadb < ${SHARE}/phpMyAdmin/sql/create_tables.sql && mariadb < /tmp/permissions.sql && rm -f /tmp/permissions.sql
